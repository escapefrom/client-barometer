...
import { finodaysClient } from "../../../api/httpClient";
...
const Account = () => {
    const { loading, data, fetch } = useApi({
        initial: [],
        fetchData: finodaysClient.users,
    });

    useEffect(() => {
        fetch().then(() => console.log(data));
    }, [fetch]);
    ...
    var accountsNew = data.map(user => {
        return {
            id: user.id,
            label: user.firstName + ' ' + user.lastName,
            balance: user.transactions.reduce((a, b) => a + b, 0)
        }
    });
